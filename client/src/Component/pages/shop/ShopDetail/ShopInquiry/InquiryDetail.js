...
const extractUserIdFromToken = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/shop/question/${questionNo}`);
                setQuestion(response.data);
                const userId = extractUserIdFromToken(); 
                setCurrentUser(userId);
                console.log("question.userId:", response.data.userId);
    ...
    const handleDeleteClick = async () => {
        if (window.confirm('정말 삭제하시겠습니까?')) {
            try {
                await axios.delete(`http://localhost:8080/shop/question/delete/${questionNo}`);
                alert('삭제 완료');
                navigate('/shop/main');
            } catch (error) {
                    ...
                    <>
                        ...
                        <div>
                            {question.userId === currentUser && (
                                <>
                                    <button onClick={() => navigate(`/inquiry/update/${questionNo}`)}>수정</button>
