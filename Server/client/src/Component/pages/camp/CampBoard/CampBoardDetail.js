...
function CampBoardDetail() {
  ...
  const handleHeart = () => {
    setLike(!like);
    axios
      .post(`http://camply.shop/camp/board/changDips`, {
        CAMP_ID: camp_id,
        USER_ID: USER_ID,
        STATUS: like,
      })
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://camply.shop/camp/board/get/${camp_id}`, {
        headers: {
      ...
      .then((response) => {
        ...
        try {
          ...
          setLoading(false);
          axios
            .post(`http://camply.shop/camp/board/checkDips`, {
              CAMP_ID: camp_id,
  ...
  const handleDelete = () => {
    ...
    if (confirmDelete) {
      axios
        .delete(`http://camply.shop/camp/board/delete/${camp_id}`)
        .then(() => {
          alert("삭제되었습니다.");
          navigate("/camp");
        })
