...
const InquiryDetail1 = () => {
    ...
    const fetchData = async () => {
      try {
        const questionResponse = await axios.get(
          `http://13.124.229.176/shop/question/${questionNo}`
        );
  ...
  const updateComments = async () => {
    try {
      const commentsResponse = await axios.get(
        `http://13.124.229.176/shop/question/comment/list/${questionNo}`
      );
  ...
  const handleDeleteClick = async () => {
    if (window.confirm("정말 삭제하시겠습니까?")) {
      try {
        await axios.delete(
          `http://13.124.229.176/shop/question/delete/${questionNo}`
        );
        alert("삭제 완료");
        navigate("/shop/main");
      } catch (error) {
